...
class RavenAgentManager:
        ...
        def _setup_client(self):
                """Configure OpenAI/Azure/Local LLM client based on provider"""
                ...
                if self.bot_doc.model_provider == "Local LLM" and self.settings.enable_local_llm:
                        ...
                        self.provider = OpenAIProvider(openai_client=client, use_responses=False)

                elif self.bot_doc.model_provider == "Azure AI" and self.settings.enable_azure_ai:
                        # Client for Azure OpenAI
                        from openai import OpenAI

                        key = self.settings.get_password("azure_api_key")
                        endpoint = (self.settings.azure_endpoint or "").rstrip("/")
                        version = (self.settings.azure_api_version or "2024-05-01-preview").strip()
                        deployment = (self.settings.azure_deployment_name or "").strip()

                        if not all([key, endpoint, version, deployment]):
                                frappe.throw(_("Azure AI settings are not complete in Raven Settings"))

                        # Important: Azure client uses base_url with deployment name
                        base_url = f"{endpoint}/openai/deployments/{deployment}"
                        client = OpenAI(api_key=key, base_url=base_url)

                        # Use responses for compatibility
                        self.provider = OpenAIProvider(openai_client=client, use_responses=True)
